...
### **گزارش کار: پیاده‌سازی الگوریتم ژنتیک و یادگیری تقویتی در LabVIEW برای بهینه‌سازی پارامترهای کمپرسور SGT-400**

---

#### **مقدمه**
در این گزارش، به پیاده‌سازی دو مدل **الگوریتم ژنتیک (Genetic Algorithm)** و **یادگیری تقویتی (Reinforcement Learning)** در محیط **LabVIEW** برای بهینه‌سازی پارامترهای کمپرسور SGT-400 پرداخته شده است. این پارامترها شامل **زاویه‌ی پره‌ها (Blade Angle)**، **نرخ جریان هوا (Airflow Rate)**، **نسبت فشار (Pressure Ratio)**، **دمای ورودی هوا (Inlet Air Temperature)** و **سرعت چرخش (RPM)** هستند. هدف از این پروژه، افزایش راندمان و کاهش مصرف انرژی در کمپرسور است.

---

### **۱. ساختار کلی برنامه در LabVIEW**

#### **۱.۱. صفحه کاربری (Front Panel)**
صفحه کاربری شامل بخش‌های زیر است:

1. **انتخاب مدل (Model Selector):**
   - یک Dropdown برای انتخاب بین دو مدل:
     - **GA:** الگوریتم ژنتیک
     - **RL:** یادگیری تقویتی

2. **ورودی پارامترها (Parameter Inputs):**
   - اسلایدرها یا ورودی‌های عددی برای تنظیم پارامترهای کمپرسور:
...
3. **تنظیمات مدل (Settings Input):**
   - تعداد نسل‌ها (برای GA)
   - تعداد مراحل آموزش (برای RL)

4. **خروجی‌ها (Output Indicators):**
   - نتایج مدل:
     - برای GA: پارامترهای بهینه
     - برای RL: عملیات انجام‌شده، وضعیت فعلی و امتیاز عملکرد
   - نمودارها برای نمایش نتایج.

---

#### **۱.۲. بلوک دیاگرام (Block Diagram)**
بلوک دیاگرام شامل مراحل زیر است:

1. **Case Structure برای انتخاب مدل:**
   - اگر **Model Selector = GA**:
     - فراخوانی SubVI مربوط به الگوریتم ژنتیک.
   - اگر **Model Selector = RL**:
     - فراخوانی SubVI مربوط به یادگیری تقویتی.

2. **SubVI برای GA (الگوریتم ژنتیک):**
   - پیاده‌سازی الگوریتم ژنتیک شامل مراحل مقداردهی اولیه، انتخاب، ترکیب و جهش.

3. **SubVI برای RL (یادگیری تقویتی):**
   - پیاده‌سازی محیط و عامل یادگیری تقویتی.

4. **نمایش نتایج (Graph Indicators):**
   - نمایش نتایج به صورت گرافیکی.

---

### **۲. پیاده‌سازی الگوریتم ژنتیک (GA)**

#### **ورودی‌ها:**
- محدوده‌ی مقادیر برای هر پارامتر:
  - زاویه‌ی پره‌ها: [10, 30]
  - نرخ جریان هوا: [50, 150]
  - نسبت فشار: [10, 20]
  - دمای ورودی هوا: [20, 60]
  - سرعت چرخش: [3000, 6000]
- تعداد نسل‌ها: 50
- اندازه‌ی جمعیت: 20
...
#### **مراحل پیاده‌سازی:**
1. **مقداردهی اولیه:**
   - از **Random Number Generator** برای تولید جمعیت اولیه استفاده شد.
   - مقادیر در محدوده‌ی مشخص‌شده برای هر پارامتر قرار داده شدند.

2. **تابع برازندگی (Fitness Function):**
   - از **Formula Node** برای محاسبه‌ی تابع برازندگی استفاده شد:
     ```plaintext
     Efficiency = 0.8 * Blade_Angle + 0.5 * Airflow_Rate - 0.2 * Pressure_Ratio + 0.3 * Inlet_Temperature + 0.1 * RPM
     ```

3. **انتخاب والدین:**
   - از **Array Operations** برای مرتب‌سازی و انتخاب بهترین افراد استفاده شد.

4. **ترکیب (Crossover):**
   - دو والد با هم ترکیب شدند تا فرزندان جدید ایجاد شوند.

5. **جهش (Mutation):**
   - تغییرات کوچکی به صورت تصادفی به فرزندان اعمال شد.

6. **تکرار:**
   - از **For Loop** برای تکرار نسل‌ها استفاده شد.

#### **خروجی:**
- پارامترهای بهینه‌ی کمپرسور.

---

### **۳. پیاده‌سازی یادگیری تقویتی (RL)**

#### **ورودی‌ها:**
- حالت اولیه سیستم:
  - دما: 50 درجه سانتی‌گراد
  - نسبت فشار: 15
  - سرعت چرخش: 4000 دور بر دقیقه
- محدوده‌ی عملیات:
  - زاویه‌ی پره‌ها: [10, 30]
  - نرخ جریان هوا: [50, 150]
  - نسبت فشار: [10, 20]
  - دمای ورودی هوا: [20, 60]
  - سرعت چرخش: [3000, 6000]
- تعداد مراحل آموزش: 100

#### **مراحل پیاده‌سازی:**
1. **محیط (Environment):**
   - از **State Machine** برای شبیه‌سازی دینامیک سیستم استفاده شد:
     ```plaintext
...
2. **آموزش عامل (Agent Training):**
   - از **MathScript Node** برای پیاده‌سازی مدل یادگیری تقویتی (مثل PPO) استفاده شد.

3. **تابع پاداش (Reward Function):**
   - از **Formula Node** برای محاسبه‌ی پاداش استفاده شد:
     ```plaintext
     Reward = 0.8 * New_Temperature - 0.2 * New_Pressure_Ratio + 0.1 * RPM
     ```

4. **ذخیره‌ی داده‌ها (Data Logging):**
   - از **Data Logging** برای ذخیره‌ی داده‌های آموزش و تست استفاده شد.

#### **خروجی:**
- عملیات انجام‌شده، وضعیت فعلی و امتیاز عملکرد.
...
### **۵. نتیجه‌گیری**
در این گزارش، دو مدل **الگوریتم ژنتیک** و **یادگیری تقویتی** برای بهینه‌سازی پارامترهای کمپرسور SGT-400 در محیط LabVIEW پیاده‌سازی شدند. این مدل‌ها به طور مؤثری می‌توانند برای افزایش راندمان و کاهش مصرف انرژی استفاده شوند. با استفاده از **Case Structure** و **SubVI**، می‌توان به راحتی بین دو مدل انتخاب کرد و نتایج را به صورت گرافیکی نمایش داد.
